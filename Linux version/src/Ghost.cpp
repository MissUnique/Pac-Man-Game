...
Ghost::Ghost(Game& g) : position({ 0, 0 }), direction(-1), prev_direction(-1), game_(g), 
        ...
        if (!canBeEaten && (target == pacman_pos)) {
            ...
            while (openlist.size() > 0) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    bool on_grid_x = (x2 >= 0 && x2 < static_cast<int>(grid.size()));
                    bool on_grid_y = (y2 >= 0 && y2 < static_cast<int>(grid[0].size()));
